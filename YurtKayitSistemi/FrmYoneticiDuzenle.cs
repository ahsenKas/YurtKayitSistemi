...
namespace YurtKayitSistemi
{
    public partial class FrmYoneticiDuzenle : Form
    {
        ...
        private void BtnSil_Click(object sender, EventArgs e)
        {
            ...
            komut.ExecuteNonQuery();
            bgl.baglanti().Close();
            MessageBox.Show("Sime işlemi gerçekleşti");
            this.adminTableAdapter.Fill(this.yurtKayitDataSet7.Admin);
        }

        private void BtnGüncelle_Click(object sender, EventArgs e)
        {
            SqlCommand komut = new SqlCommand("update admin set YöneticiAd=@p1,YöneticiSifre=@p2  where Yoneticiid=@p3", bgl.baglanti());
            komut.Parameters.AddWithValue("@p1", TxtKullaniciAd.Text);
            komut.Parameters.AddWithValue("@p2", TxtSifre.Text);
